...
void CSCardDemoDlg::OnReset() 
{
	...
	{
		SetDlgItemText(IDS_MSG, szATR);				// Display the ATR Information
	}
}
...
void CSCardDemoDlg::OnCmd() 
{
	...
	::GetDlgItemText(this->m_hWnd, IDE_APDU, szApdu, 1024);
	if(strlen(szApdu) <=0)
	{
	...
	if(m_SCard.SCardTransmit(szApdu, szResp, &nSW))	// Do Apdu Transmit
	{
		strResp.Format("%s(%04X)", szResp, nSW);
		SetDlgItemText(IDS_MSG, strResp);			// Show it
	}
