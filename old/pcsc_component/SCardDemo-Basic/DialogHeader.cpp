...
void CDialogHeader::SetBMPRes(const UINT rID)
{
	m_bmpID = rID;

	//Set new header bitmap
	if(rID != -1)
	{
		if(!m_bmpHead.LoadBitmap(rID))
			m_bmpID = -1;			// bitmap load failed.

		// the the width & height.
		BITMAP		bmp;
...
void CDialogHeader::OnPaint() 
{
	...
	CRect		rect, rcEffect;		//General purpose rect
	...
	//dc.Draw3dRect(rcEffect,GetSysColor(COLOR_BTNHIGHLIGHT),GetSysColor(COLOR_BTNSHADOW));
	dc.Draw3dRect(rcEffect, GetSysColor(COLOR_3DSHADOW), GetSysColor(COLOR_3DHIGHLIGHT));

	//dc.MoveTo(rect.left, rect.top + rect.Height());
	//dc.LineTo(rect.right, rect.top + rect.Height());
	//dc.MoveTo(rect.left, rect.top + rect.Height()+1);
	//dc.LineTo(rect.right, rect.top + rect.Height()+1);
/*
...
*/
	...
	if (-1 != m_bmpID)
	{
		...
		CBitmap *oldBmp = dcBmp.SelectObject(&m_bmpHead);
